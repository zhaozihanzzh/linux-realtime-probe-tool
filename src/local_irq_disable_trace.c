...
DEFINE_PER_CPU(unsigned int, local_nr_entries);
DEFINE_PER_CPU(unsigned long *, local_entries); // 关中断时的堆栈信息
DEFINE_PER_CPU(struct files_struct*, local_files); // 文件
// 链表
// 头结点数据域不使用
...
static void irqoff_handler(void *none, unsigned long ip, unsigned long parent_ip) {
	...
	*this_cpu_ptr(&local_tracing) = true;
	...
	// 保存堆栈
	*this_cpu_ptr(&local_entries) = kmalloc(MAX_STACK_TRACE_DEPTH * sizeof(*local_entries), GFP_ATOMIC);
    if (*this_cpu_ptr(&local_entries)) {
        static struct stack_trace local_trace;
        local_trace.nr_entries = 0;
        local_trace.max_entries = MAX_STACK_TRACE_DEPTH;
        local_trace.entries = *this_cpu_ptr(&local_entries);
        local_trace.skip = 0;
        save_stack_trace_tsk(get_current(), &local_trace);
        *this_cpu_ptr(&local_nr_entries) = local_trace.nr_entries;
    }
...
static void irqon_handler(void *none, unsigned long ip, unsigned long parent_ip) {
	...
	*this_cpu_ptr(&local_tracing) = true;
	...
	if (likely(*this_cpu_ptr(&has_off_record))) {
		time64_t local_duration = (enable_local_irq_time.tv_sec - *this_cpu_ptr(&disable_local_irq_time.tv_sec)) * 1000000000ll + \
		...
		if (local_duration  > nsec_limit) {
            ...
            local_list_node->entries = *this_cpu_ptr(&local_entries);
            local_list_node->nr_entries = *this_cpu_ptr(&local_nr_entries);
